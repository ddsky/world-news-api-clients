...
/// struct for typed errors of method [`newspaper_front_pages`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NewspaperFrontPagesError {
    Status401(),
    Status402(),
    Status403(),
    Status404(),
...
pub async fn newspaper_front_pages(configuration: &configuration::Configuration, source_country: Option<&str>, source_name: Option<&str>, date: Option<&str>) -> Result<models::NewspaperFrontPages200Response, Error<NewspaperFrontPagesError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<NewspaperFrontPagesError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
