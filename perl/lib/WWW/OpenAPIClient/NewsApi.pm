...
# @param string $date The date for which the front page should be retrieved. (optional)
{
    my $params = {
    'source_country' => {
        data_type => 'string',
    ...
    'date' => {
        data_type => 'string',
        description => 'The date for which the front page should be retrieved.',
        required => '0',
    },
    };
    __PACKAGE__->method_documentation->{ 'newspaper_front_pages' } = {
        summary => 'Newspaper Front Pages',
        params => $params,
        returns => 'NewspaperFrontPages200Response',
        };
}
# @return NewspaperFrontPages200Response
#
sub newspaper_front_pages {
    my ($self, %args) = @_;

    # parse inputs
    my $_resource_path = '/front-pages';
    ...
    my $_response_object = $self->{api_client}->deserialize('NewspaperFrontPages200Response', $response);
    return $_response_object;
}
