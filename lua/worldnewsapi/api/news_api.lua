...
local worldnewsapi_newspaper_front_pages_200_response = require "worldnewsapi.model.newspaper_front_pages_200_response"
...
function news_api:newspaper_front_pages(source_country, source_name, date)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/front-pages?source-country=%s&source-name=%s&date=%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return worldnewsapi_newspaper_front_pages_200_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
